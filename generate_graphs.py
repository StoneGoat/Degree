...
def parse_zap_data(zap_report):
    ...
    try:
        for alertitem in zap_report.findall('.//alertitem'):
            ...
            site_element = None; current = alertitem
            for _ in range(3):
                parent = current.find('..');
                if parent is not None and parent.tag == 'site' and 'name' in parent.attrib: site_element = parent; break
                if parent is None: break; current = parent
...
def parse_nmap_data(nmap_results_root):
    ...
    try:
        for host_ip_tag in nmap_results_root:
             host_data_tag = host_ip_tag.find(host_ip_tag.tag)
             if host_data_tag is None:
                 if host_ip_tag.find('.//addresses/ipv4') is not None: host_data_tag = host_ip_tag
             ...
             ip_elem = host_data_tag.find('.//addresses/ipv4');
             if ip_elem is not None and ip_elem.text: host_info['ip'] = ip_elem.text
             hostname_elem = host_data_tag.find('.//hostnames/name');
             if hostname_elem is not None and hostname_elem.text: host_info['hostname'] = hostname_elem.text
             status_elem = host_data_tag.find('.//status/state');
             if status_elem is not None and status_elem.text: host_info['status'] = status_elem.text
             open_ports_tag = host_data_tag.find('.//open_ports')
             if open_ports_tag is not None:
                 for port_tag in open_ports_tag:
                     port_number_str = port_tag.tag.split('_')[-1]
                     if port_number_str.isdigit():
                         port_number = port_number_str
                         port_info = {'number': port_number, 'protocol': 'tcp', 'state': port_tag.findtext('state', 'Unknown'), 'name': port_tag.findtext('name', 'Unknown'), 'product': port_tag.findtext('product', ''), 'version': port_tag.findtext('version', ''), 'extrainfo': port_tag.findtext('extrainfo', '')}
                         host_info['ports'].append(port_info)
                     else: print(f"  - Warning: Could not extract port number from Nmap port tag '{port_tag.tag}'")
...
def parse_nikto_data_from_output(raw_output):
    ...
    findings = []
    try:
        finding_pattern = re.compile(r"^\+\s+(?!Start\sTime:|End\sTime:|Nikto\sversion|\d+\s+hosts? tested|Server:|Target\sIP:|Target\sHostname:|Target\sPort:)(.*)", re.MULTILINE)
        ...
        if not findings:
           ...
           for line in lines:
               line = line.strip()
               if line.startswith('+ ') and not any(header in line for header in ['Start Time:', 'End Time:', 'Nikto version', 'host(s) tested', 'Server:', 'Target IP:', 'Target Hostname:', 'Target Port:']):
...
def visualize_zap_risk_distribution(zap_data, output_file='1_zap_risk_distribution.png'):
    ...
    try:
        ...
        try: plt.savefig(output_file, bbox_inches='tight'); print(f"  - Saved ZAP unique risk distribution chart to {output_file}")
...
def visualize_alert_counts(zap_data, output_file='2_zap_alert_counts.png'):
    ...
    try:
        ...
        try: plt.savefig(output_file); print(f"  - Saved ZAP alert counts by occurrence chart to {output_file}")
...
def visualize_nmap_port_status(nmap_data, output_file='4_nmap_port_status.png'):
    ...
    try:
        ...
        try: plt.savefig(output_file); print(f"  - Saved Nmap port status chart to {output_file}")
...
def visualize_nikto_findings(nikto_data, output_file='5_nikto_findings.png'):
    ...
    try:
        ...
        try: plt.savefig(output_file, bbox_inches='tight'); print(f"  - Saved Nikto findings chart to {output_file}")
    ...
    try:
        ...
        if security_data.get('zap'):
            ...
            for alert in zap_alerts:
                ...
                if plugin_id not in processed_plugin_ids:
                    risk_code = alert.get('risk_code', -1); unique_alerts_by_risk[risk_code] += 1
                    processed_plugin_ids.add(plugin_id)
            h = unique_alerts_by_risk.get(3, 0); m = unique_alerts_by_risk.get(2, 0)
            l = unique_alerts_by_risk.get(1, 0); i = unique_alerts_by_risk.get(0, 0) + unique_alerts_by_risk.get(-1, 0)
            zap_total_unique = h + m + l + i
            total_findings_display.append(zap_total_unique)
            high_risk.append(h); medium_risk.append(m); low_risk.append(l); info_risk.append(i)
            print(f"  - ZAP Summary (Unique Types): H={h}, M={m}, L={l}, I={i} (Total Unique: {zap_total_unique})")
        else:
             has_other_tools = security_data.get('nmap') or security_data.get('nikto')
             if has_other_tools:
                  tool_names.append('OWASP ZAP'); total_findings_display.append(0)
                  high_risk.append(0); medium_risk.append(0); low_risk.append(0); info_risk.append(0)
                  print("  - ZAP Summary: No data found.")

        # --- Nmap Summary (Counts OPEN ports) ---
        if security_data.get('nmap'):
            ...
            print(f"  - Nmap Summary: Open Ports={open_ports_count} -> H={n_high}, M={n_med}, L={n_low}, I={n_info}")
             ...
             has_other_tools = security_data.get('zap') or security_data.get('nikto')
             if has_other_tools:
                  tool_names.append('Nmap'); total_findings_display.append(0)
                  high_risk.append(0); medium_risk.append(0); low_risk.append(0); info_risk.append(0)
                  print("  - Nmap Summary: No data found.")

        # --- Nikto Summary (Counts unique findings) ---
        if security_data.get('nikto') and security_data['nikto'].get('findings'):
            tool_names.append('Nikto')
            findings = security_data['nikto']['findings']; nikto_total = len(findings)
            total_findings_display.append(nikto_total)
            high_keywords = ['vulnerable', 'osvdb-', 'cve-', 'edb-id', ' xss', 'injection', 'attack', 'exploit']
            medium_keywords = ['outdated', 'default file', 'backup file', 'directory indexing', 'password']
            low_keywords = ['x-frame-options', 'x-content-type-options', 'strict-transport-security', 'cookie', 'httponly', 'server header', 'x-powered-by']
            ...
            has_other_tools = security_data.get('zap') or security_data.get('nmap')
            if has_other_tools:
                  tool_names.append('Nikto'); total_findings_display.append(0)
                  high_risk.append(0); medium_risk.append(0); low_risk.append(0); info_risk.append(0)
                  print("  - Nikto Summary: No data found.")
        ...
        np_info = np.array(info_risk); np_low = np.array(low_risk); np_med = np.array(medium_risk); np_high = np.array(high_risk)
        totals_for_label = np.array(total_findings_display)

        if not (len(x) == len(np_info) == len(np_low) == len(np_med) == len(np_high) == len(totals_for_label)):
             print("  - Error: Mismatch in array lengths for summary plotting. Skipping."); plt.close(); return
        ...
        try: plt.savefig(output_file, bbox_inches='tight'); print(f"  - Saved Overall Summary chart to {output_file}")
...
def main(xml_file_path, output_dir):
    ...
    try:
        ...
        # zap_confidence_output_path = os.path.join(output_dir, '3_zap_confidence_vs_risk.png') # Path if function were used
        ...
        if security_data.get('zap'):
            ...
            # visualize_confidence_vs_risk(security_data['zap'], output_file=zap_confidence_output_path) # Pass path if using
            print(f"  - Skipping ZAP Confidence vs Risk heatmap.") # Keep skip message if not generating
        ...
        print(f"Please check the directory '{output_dir}' for the generated PNG files.") # Updated message
...
def generate_all_graphs_for_scan(scan_id):
    ...
    """
    # Assumes the XML report is in the script's CWD or a known relative path
    xml_filename = f"scan-report{scan_id}.xml"
    # Defines the output directory structure relative to the script's location
    # This MUST match where the Flask app expects images.
    ...
    # Create directory (might be redundant if Flask already did, but safe)
    try:
        os.makedirs(output_dir, exist_ok=True)
    except OSError as e:
        print(f"Error creating output directory '{output_dir}' in generate_graphs: {e}")
        return None # Return None if directory cannot be made

    # Check if input XML exists before calling main logic
    if not os.path.exists(xml_filename):
         print(f"Error: Input XML '{xml_filename}' not found for graph generation.")
         # Optionally: Could create placeholder images or skip? For now, just return None.
         return None

    # Call the main logic function
    main(xml_filename, output_dir)
    print(f"--- Graph Generation Potentially Complete for Scan ID: {scan_id} ---")
    return output_dir # Return the directory where graphs were saved

# --- Keep this block to allow running generate_graphs.py directly ---
if __name__ == "__main__":
    ...
    # Call the new callable function when run as a script
